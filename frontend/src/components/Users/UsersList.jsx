...
const UsersList = (props) => {
  ...
  const deleteMapping = async () => {
    ...
    try {
      await axios
        .delete(`http://localhost:8080/user/${selectedDeleteProductId}`)
        .then((res) => {
  ...
  const tableData = () => {
    const rowData = [];
    const filteredItems = usersList || [];

    filteredItems.length > 0 &&
      filteredItems.map((menu, i) => {
        rowData.push({
          Sno: i + 1,
          Name: menu?.user_name,
          Email: menu?.user_email,
          Role: menu?.user_role,
          id: menu?.user_id,
        });
      });
    ...
    const columnDefs = [];
    var x = ["Sno", "Name", "Email", "Role", "Action"];
    x.map((col) => {
      var y = {
        colId: col, 
        field: col,
      ...
      <>
        <span title="Edit">
          <Edit2
            ...
            title="Edit"
            onClick={(e) => {
              navigate("/add-user", { state: { id: props.data.id } });
            }}
          />
        </span>
        <Trash
    ...
    <Container>
      ...
      <div className="mb-3">
        <Card className="ag-theme-alpine p-3">
          {usersList ? tableData() : renderLoader()}
        </Card>
      </div>
